...
function parseArgs(argv) {
  const opts = { format: "summary", phase: "", unregistered: false, missingTests: false };
...
async function listSystemFiles() {
  const out = [];
  for await (const entry of Deno.readDir(SYSTEM_DIR)) {
    if (entry.isFile && entry.name.endsWith(".js")) out.push(`${SYSTEM_DIR}/${entry.name}`);
  }
  return out.sort();
}
...
function parseImports(text) {
  ...
  while ((match = re.exec(text))) {
    ...
    const spec = match[2];
    if (!spec.includes("/systems/") && !spec.startsWith("../rules/systems/")) continue;
    const file = spec.split("/").at(-1);
    for (const name of names) imports.set(name, file.endsWith(".js") ? file : `${file}.js`);
  }
  return imports;
}

function parseRegistrations(text, imports) {
  const rows = [];
...
async function buildRows() {
  ...
  const registeredNames = new Set(registered.map((row) => row.file).filter(Boolean));
  const allTests = [];
  ...
  for (const row of registered) {
    ...
    const obviousTest = await fileExists(`${TEST_DIR}/${base}.test.mjs`) ||
      allTests.some((name) => name.toLowerCase().includes(base.toLowerCase().replace(/system$/, "")));
    rows.push({
      kind: "registered",
      phase: row.phase,
      name: row.name,
  ...
  for (const path of systemFiles) {
    ...
    rows.push({
      kind: importedFiles.has(file) ? "imported-not-registered" : "unregistered",
      phase: "",
...
function csv(rows) {
  return [
    ...
    ...rows.map((r) => [r.kind, r.phase, r.name, r.file, r.imported, r.registered, r.obviousTest].join(",")),
  ].join("\n") + "\n";
}
...
function summary(rows) {
  ...
  for (const row of registered) phases.set(row.phase, (phases.get(row.phase) || 0) + 1);
  ...
  lines.push(`unregistered/imported-not-registered files: ${unregistered.length}`);
  lines.push(`registered systems without obvious tests: ${missingTests.length}`);
  lines.push("");
  lines.push("phase counts:");
  for (const [phase, count] of [...phases.entries()].sort()) lines.push(`  ${phase}: ${count}`);
  lines.push("");
  ...
  for (const row of missingTests.slice(0, 40)) lines.push(`  ${row.phase}: ${row.name} (${row.file})`);
  return lines.join("\n") + "\n";
}
...
async function main() {
  ...
  if (opts.missingTests) rows = rows.filter((r) => r.kind === "registered" && r.obviousTest === "no");
