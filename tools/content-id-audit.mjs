...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if ((arg === "--id" || arg === "-i") && argv[i + 1]) opts.id = String(argv[++i]);
...
async function collectFiles(root, out = []) {
  ...
  for await (const entry of Deno.readDir(root)) {
    ...
    else if (entry.isFile && /\.(js|mjs|json|md)$/.test(entry.name)) out.push(path);
  }
  return out;
}

function classify(file, line, id) {
  if (/define(Item|Monster|Spell)\s*\(/.test(line) || new RegExp(`\\b${id}\\s*:`).test(line)) return "definition";
  if (file.includes("/palette/") || /glyph|fg|bg|color|sprite/i.test(line)) return "visual";
  if (/loot|drop|table|spawn|pickMonster|materialize/i.test(file + " " + line)) return "spawn-loot";
...
async function main() {
  ...
  for (const file of files) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      rows.push({ kind: classify(file, lines[i], opts.id), file, line: i + 1, text: lines[i].trim() });
  ...
  console.log(`matches: ${rows.length}${rows.length >= opts.limit ? ` (limited to ${opts.limit})` : ""}`);
