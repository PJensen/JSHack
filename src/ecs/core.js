...
export function defineComponent(name, defaults){
  ...
  return Object.freeze({ key, name, defaults: shape });
}
...
export class World {
  ...
  *queryGen(...terms){
  ...
  system(fn, phase='update'){ if (this._debug) console.debug(`[ECS] system added @${phase}`); this._systems[phase].push(fn); return this; }
  ...
  tick(dt){
    ...
    for (const ph of ['early','update','late','render']){
      ...
      const list = P[ph];
      for (let i=0;i<list.length;i++) list[i](this, dt);
...
// // Optional autorun in browser if requested
// if (typeof window !== 'undefined' && (window.ECS_SELF_TEST === true)) {
//   try { runSelfTests(); } catch(e){ console.error('[ECS] self-tests fatal', e); }
// }
//     if (opts.renderEveryFrame){ const alpha = acc / fixed; for (const f of world._systems.render) f(world, alpha); }
//     req = requestAnimationFrame(frame);
//   }
...
/** Example: simple movement system (drop‑in) */
export function MovementSystem(world, dt){
  const Position = MovementSystem.Position;
  const Velocity = MovementSystem.Velocity;
  if (!Position || !Velocity) return;
  for (const [id, p, v] of world.query(Position, Velocity)){
    p.x += v.x * dt; p.y += v.y * dt;
  }
}
MovementSystem.Position = defineComponent('Position', {x:0,y:0});
MovementSystem.Velocity = defineComponent('Velocity', {x:0,y:0});
