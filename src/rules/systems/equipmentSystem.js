...
function applyBonuses(acc, bonuses) {
  ...
  if (Number.isFinite(bonuses.hungerRate)) acc.hungerRateDerived += bonuses.hungerRate;
}
...
export function equipmentSystem(world) {
  for (const [id, eq] of world.query(Equipment)) {
    ...
    for (const slot of NON_AMMO_GEAR_SLOTS) {
      ...
      const ctx = {
        addBonus: (k, v) => {
          ...
          else if (k === 'hungerRate') d.hungerRateDerived += v;
        },
        entityId: id,
        itemId,
        world
    ...
    eq.hungerRateDerived = d.hungerRateDerived;
  }
}
