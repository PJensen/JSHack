...
import {
  listSleepProfileIds,
  resolveSleepProfile,
} from "../src/rules/data/sleepProfiles.js";
...
Deno.test("SleepState blocks queued actor intents", () => {
  ...
  assert(
    ...
    "sleeping actor should lose MoveIntent",
  );
  assert(
    !world.has(actor, AttackIntent),
    "sleeping actor should lose AttackIntent",
  );
  assert(
    !world.has(actor, SearchIntent),
    "sleeping actor should lose SearchIntent",
  );
...
Deno.test("putActorToSleep emits only on awake to asleep transition", () => {
  ...
  assertEquals(
    putActorToSleep(world, actor, { reason: "test", wakeDifficulty: 12 }),
    true,
  );
  ...
  assertEquals(
    putActorToSleep(world, actor, { reason: "test_again", wakeDifficulty: 4 }),
    false,
  );
  assertEquals(events.length, 1);
...
Deno.test("tryWakeActor respects non-damage wake difficulty", () => {
  ...
  assertEquals(
    tryWakeActor(world, sleeper, { reason: "noise", intensity: 4 }),
    false,
  );
  assert(isAsleep(world, sleeper), "weak noise should not wake sleeper");

  assertEquals(
    tryWakeActor(world, sleeper, { reason: "noise", intensity: 8 }),
    true,
  );
...
Deno.test("WorldView projects sleeping tag while SleepState is asleep", () => {
  ...
  assert(
    rec.tags.includes(SLEEP_DISPLAY_TAG),
    "asleep entity should carry sleeping tag",
  );

  world.set(sleeper, SleepState, {
    asleep: false,
    wakeDifficulty: 8,
    wakeRadius: 2,
    wakeOnDamage: true,
  ...
  assert(
    !rec.tags.includes(SLEEP_DISPLAY_TAG),
    "awake entity should not carry sleeping tag",
  );
});
...
Deno.test("sleep profiles expose activity-pattern vocabulary", () => {
  ...
  const roost = resolveSleepProfile({
    pattern: "nocturnal",
    context: "roost",
    chance: 0.25,
  });
...
Deno.test("authored bat sleep chance attaches SleepState deterministically", () => {
  ...
  assert(
    isAsleep(sleepyWorld, sleepyBat),
    "bat should spawn asleep when roll is below chance",
  );
  ...
  assert(
    !isAsleep(awakeWorld, awakeBat),
    "bat should spawn awake when roll is above chance",
  );
});
...
Deno.test("authored dragon sleep is forwarded through spawn params", () => {
  ...
  assert(
    sleep?.asleep === true,
    "dragon should spawn asleep from authored params",
  );
...
Deno.test("cave bear den sleep follows time of day at spawn", () => {
  ...
  assert(
    !isAsleep(dayWorld, dayBear),
    "cave bear should spawn awake during active daytime phase",
  );
  ...
  assert(
    isAsleep(nightWorld, nightBear),
    "cave bear should spawn asleep during night rest phase",
  );
});
...
Deno.test("sleep schedule wakes cave bear at dawn", () => {
  ...
  world.step = SIX_AM;
  const bear = spawnMonsterEntity(world, {
    identity: "cave_bear",
    sleep: false,
  });
  world.add(bear, SleepState, {
    asleep: true,
    wakeDifficulty: 8,
    wakeRadius: 2,
    wakeOnDamage: true,
...
Deno.test("sleep schedule does not put aggroed cave bear to sleep at night", () => {
  ...
  assert(
    !isAsleep(world, bear),
    "aggroed cave bear should not fall asleep when night starts",
  );
});

Deno.test("sleep schedule ignores controlled pets", () => {
  const world = new World({ seed: 0x7A4ED });
  world.step = NINE_PM;
  const bear = spawnMonsterEntity(world, {
    identity: "cave_bear",
    sleep: false,
  ...
  assert(
    !isAsleep(world, bear),
    "controlled pets should not enter authored sleep cycles",
  );
  assertEquals(slept, null);
});
...
Deno.test("sleep schedule can put unaware cave bear to sleep at night", () => {
  ...
  assert(
    isAsleep(world, bear),
    "unaware cave bear should fall asleep during night rest phase",
  );
