...
import { createLocalEmitterAmbientController, computeCookingFireLoopVolume, computeHolySiteLoopVolume, computeTorchLoopVolume, computeWoodcutterLoopVolume, computePortalLoopVolume } from "../src/display/audio/localEmitterAmbientController.js";
...
Deno.test("local emitter loop curves fall off by radius", () => {
  ...
  assertAlmostEquals(computePortalLoopVolume(1), 0.6, 1e-10);
  ...
  assertEquals(computePortalLoopVolume(8), 0);
  ...
  assert(computePortalLoopVolume(3) < computePortalLoopVolume(2));
});
...
Deno.test("local emitter controller follows portal idle loops", () => {
  const calls = [];
  const controller = createLocalEmitterAmbientController({
    resolveFn(id) {
      return { url: `./assets/audio/${id}.mp3`, bus: "ambient" };
  ...
  controller.syncWorldView({
    player: { pos: { x: 10, y: 10 } },
    audioEmitters: [{ profile: "portal_idle", pos: { x: 11, y: 10 }, interior: false }],
    entities: [],
  });
  assertEquals(calls[0]?.type, "start");
  assertEquals(calls[0]?.url, "./assets/audio/ambient:portal_idle.mp3");
  assertEquals(calls[0]?.opts?.bus, "ambient:loop");
  assertAlmostEquals(calls[0]?.opts?.volume, 0.6, 1e-10);

  controller.syncWorldView({
    player: { pos: { x: 10, y: 10 } },
    audioEmitters: [{ profile: "portal_idle", pos: { x: 14, y: 10 }, interior: false }],
    entities: [],
  });
  assertEquals(calls[1]?.type, "set");
  assertEquals(calls[1]?.url, "./assets/audio/ambient:portal_idle.mp3");
  assert(calls[1]?.volume < calls[0]?.opts?.volume);
  ...
  assertEquals(calls[2]?.type, "stop");
  assertEquals(calls[2]?.url, "./assets/audio/ambient:portal_idle.mp3");
});
