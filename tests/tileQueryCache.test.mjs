...
import { getLivingEntityAt, getItemsAt, forEachAt, forEachItemAt, invalidateTileQueryCache } from "../src/rules/utils/tileQueryCache.js";
...
Deno.test("tileQueryCache: living and item lookups by tile", () => {
  ...
  const itemSeen = [];
  forEachItemAt(world, 3, 4, (id) => itemSeen.push(id));
  assert(itemSeen.includes(item), "forEachItemAt should include item entity");
