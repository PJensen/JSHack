...
Deno.test("rat quest records genocide as an alternate resolution", () => {
  ...
  const advanced = [];
  world.on("quest:advanced", (payload) => advanced.push(payload));
  ...
  const vars = world.get(questId, QuestVars)?.data || {};
  assertEquals(world.get(questId, QuestState)?.node, "report");
  assertEquals(vars.ratsGenocided, true);
  assertEquals(vars.resolution, "genocide");
  assertEquals(vars.worldOutcome, "rats_erased");
  assertEquals(vars.objective, "Return to the barkeep.");
  assertEquals(advanced.at(-1)?.objective, "Return to the barkeep.");
});

Deno.test("rat quest ignores unrelated or premature genocide events", () => {
  ...
  world.tick(0);
  assertEquals(world.get(questId, QuestState)?.node, "offer");
  ...
  const vars = world.get(questId, QuestVars)?.data || {};
  assertEquals(world.get(questId, QuestState)?.node, "hunt");
  assertEquals(vars.ratsGenocided, false);
  assertEquals(vars.resolution, "");
});
